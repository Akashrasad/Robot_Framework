...
if(u_algo == 1)     // max limit
{
    float min_in = 0.0f, max_in = 1.0f;
    float min_out = endAngle, max_out = startAngle;
    ...
    // calc visible area
    if(angDeg > expected_ang || angDeg < endAngle)
        discard;

    color = texColor;
}
else if(u_algo == 2)     // min limit
{
    float min_in = 0.0f, max_in = 1.0f;
    ...
    // calc visible area
    if(angDeg < expected_ang || angDeg > startAngle)
        discard;
