...
class weCanSim:
    ...
    def swc_select(self,name):
        print("Location of buttons and names---->:",name)
        if name == 'swc_down':
            print("Inside swc_down")
            self.exctdll.wecan_swc_down()
        elif name == 'swc_up':
            print("Inside swc_up")
            self.exctdll.wecan_swc_up()
        elif name == 'swc_left':
            print("Inside swc_left")
            self.exctdll.wecan_swc_left()
        elif name == 'swc_right':
            print("Inside swc_right")
            self.exctdll.wecan_swc_right()
        elif name == 'swc_select':
            print("Inside swc_select")
            self.exctdll.wecan_swc_select()
    

    def getsignal(self,signal) -> int:
        b_string = signal.encode('utf-8')
        print("signal perioicity-->", self.exctdll.wecan_periodicrate(b_string))
        return self.exctdll.wecan_periodicrate(b_string)
    ...
    def gb_default(self):
        default_signals=[]
        '''entry ='{"Type" : "Signal","Mode" : "HS","name" : "IAutoFanCstStAvl","Value" :"1"}'
        default_signals.append(entry)'''
        print("Signal Commented")
        for signal in default_signals:
            print(signal)
            b_string = signal.encode('utf-8')
            self.exctdll.wecan_send(b_string)
